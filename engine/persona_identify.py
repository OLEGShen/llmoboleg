...
def identify(person, candidate_num=10, fast=False):
    ...
    att_hub = []
    iter_roles = list(roles.items())[:2] if fast else roles.items()
    for role, description in iter_roles:
        role = role.split("#")[0]
        ...
        try:
            ...
            continue
    scores_dict = score_from_rating(person, att_hub, e_template1, metric="rate", neg_routines=neg_routines, fast=fast)
...
def score_from_rating(person, att_hub, e_template, metric, neg_routines=None, fast=False):
    scores_dict = {}
    r = 0
    for att in att_hub:
        r += 1
        max_samples = 5 if fast else 30
        for i in range(min(max_samples, len(person.train_routine_list))):
